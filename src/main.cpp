...
// Initialize phi as a non-negative Gaussian tube around a line segment
// phi = exp(-(dist^2) / (2*sigma^2)) with optional amplitude
static void init_phi_line_segment_gaussian(MultiFab& phi, const Geometry& geom,
                                           Real x0, Real y0, Real z0,
                                           Real x1, Real y1, Real z1,
                                           Real sigma, Real amplitude)
{
    const auto dx = geom.CellSize();
    const auto plo = geom.ProbLo();

    Real inv2sig2 = 1.0 / (2.0 * sigma * sigma);
    ...
    for (MFIter mfi(phi); mfi.isValid(); ++mfi) {
        ...
        ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
            ...
            Real d = distance_point_to_segment(x, y, z, x0, y0, z0, x1, y1, z1);
            Real val = amplitude * std::exp( - (d*d) * inv2sig2 );
            p(i,j,k) = val; // non-negative "fire" source
        });
    }
}
// ================================================================
...
int main(int argc, char* argv[])
{
    amrex::Initialize(argc, argv);
    {
        ...
        std::string source_type = "line"; // "line" or "sphere"
        pp.query("source_type", source_type);

        // Line segment parameters
        Real ls_x0 = 0.0, ls_y0 = 0.0, ls_z0 = 0.0;   // start point
        Real ls_x1 = 0.0, ls_y1 = 0.0, ls_z1 = 1.0;   // end point (e.g., vertical line)
        Real ls_radius = 0.25;                        // for SDF cylinder
        Real ls_sigma  = 0.10;                        // for Gaussian tube
        Real ls_amp    = 1.0;                         // for Gaussian tube amplitude

        pp.query("line_start_x", ls_x0);
        pp.query("line_start_y", ls_y0);
        pp.query("line_start_z", ls_z0);
        pp.query("line_end_x",   ls_x1);
        pp.query("line_end_y",   ls_y1);
        pp.query("line_end_z",   ls_z1);
        pp.query("line_radius",  ls_radius);
        pp.query("line_sigma",   ls_sigma);
        pp.query("line_amplitude", ls_amp);
        ...
        if (source_type == "line") {
            ...
            // (B) Gaussian tube around a segment (uncomment to use instead):
            init_phi_line_segment_gaussian(phi, geom, ls_x0, ls_y0, ls_z0,
                                            ls_x1, ls_y1, ls_z1, ls_sigma, ls_amp);
