...
int main(int argc, char* argv[])
{
  amrex::Initialize(argc, argv);
  {
    ...
    if (use_levelset)
      ...
      // FARSITE: compute initial ROS field to obtain a CFL-based adaptive dt
      // (mirrors the levelset/MTT initialisation paths above).
      apply_wind_terrain_effective_velocity(vel_effective, vel, terrain_slopes.get(), inputs);
      if (heat_flux_active) {
          apply_heatflux_wind(vel_effective, vel, heat_flux_mf, &phi, inputs.heat_flux);
      }
      ...
      dt = compute_dt(R_mf, geom, inputs.cfl);
      amrex::Print() << "Using FARSITE propagation; initial adaptive dt = " << dt << " s (CFL-based)\n";
    }
	...
	// Adaptive dt: recompute CFL-based timestep from current ROS field.
	dt = compute_dt(R_mf, geom, inputs.cfl);
