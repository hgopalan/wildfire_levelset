...
The Weise & Biging (1996) fire whirl model is an **optional diagnostic sub-model** that computes fire whirl characteristics from existing fire behavior outputs. Unlike Rothermel, Balbi, or Cheney–Gould, it does **not** replace or modify the primary fire spread model; instead, it runs **alongside** any spread model to predict whirl formation when enabled via `weise_biging.enable = 1`.

### Physical basis

Fire whirls are columnar vortices generated by fire-induced updrafts interacting with ambient wind and terrain. The model characterises the whirl geometry and kinematics using:

1. **Flame tilt angle** (Weise & Biging 1996 empirical):
   ```
   tan θ = 0.382 × Fr^0.432 + tan β
   ```
   where `Fr = U² / (g × L_f)` is the modified Froude number, `U` is wind speed [m/s], `g = 9.81 m/s²`, `L_f` is Byram flame length [m], and `β` is terrain slope angle.

2. **Vertical flame height**:
   ```
   H_f = L_f × cos θ
   ```

3. **Fire whirl geometry** (Rankine-vortex scaling):
   ```
   H_w = H_f                               (whirl height equals vertical flame height)
   r_w = max(c_r × H_w, 0.01 m)            (whirl core radius)
   Γ   = U × H_w                           (circulation)
   Ω   = Γ / (2π × r_w²)                   (angular velocity [rad/s])
   v_θ = Ω × r_w                           (maximum tangential velocity [m/s])
   ```

### Inputs

The model consumes outputs from `compute_fire_behavior()`:

| Input | Source | Description |
|-------|--------|-------------|
| `fireline_intensity_mf` | Byram (1959) | Fireline intensity [kW/m] |
| `flame_length_mf` | Byram (1959) | Flame length [m] |
| `vel` | Wind field | Wind velocity [m/s] |
| `terrain_slopes` | Optional | Slope components tan(β_x), tan(β_y) |

Cells with `I_B ≤ I_B_min` are zeroed to suppress spurious predictions outside active burning regions.

### Configuration via parmparse (`weise_biging.*`)

| Parameter | Default | Description |
|-----------|---------|-------------|
| `weise_biging.enable` | `0` | Enable (1) or disable (0) the fire whirl model |
| `weise_biging.c_r` | `0.1` | Whirl core radius-to-height ratio (dimensionless) |
| `weise_biging.I_B_min` | `1.0` kW/m | Minimum Byram fireline intensity threshold |

### Outputs

Six diagnostic fields are written to every plotfile when enabled:

| Field | Units | Description |
|-------|-------|-------------|
| `weise_flame_height` | m | Vertical component of tilted flame |
| `weise_flame_tilt` | rad | Flame tilt angle from vertical |
| `weise_whirl_height` | m | Fire whirl column height |
| `weise_whirl_radius` | m | Fire whirl core radius |
| `weise_angular_velocity` | rad/s | Whirl angular velocity |
| `weise_max_tang_vel` | m/s | Maximum tangential velocity at core edge |

### Example inputs file snippet

```ini
# Enable Weise-Biging fire whirl diagnostics
weise_biging.enable  = 1
weise_biging.c_r     = 0.1     # 10% radius-to-height ratio
weise_biging.I_B_min = 1.0     # 1 kW/m minimum intensity threshold

# Works with any fire spread model:
fire_spread_model = rothermel  # or balbi, or cheney_gould
propagation_method = farsite
rothermel.fuel_model = FM4
...
u_x = 8.0
u_y = 0.0
rothermel.terrain_file = terrain.xyz  # slope β comes from here if present
```
